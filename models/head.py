...
from models.common import Conv, DFL
from utils.general import make_anchors, dist2bbox
...
class DetectionHead(nn.Module):
    ...
    def __init__(self, n_classes: int = 1, in_channels: List[int] = []):
        super().__init__()
        self.nc = n_classes
        ...
        self.f = -1  # Add this line to define the 'f' attribute
    ...
    def _bias_init(self) -> None:
        ...
        for b_list, c_list, s in zip(self.box_convs, self.cls_convs, self.stride):
            b_list[-1].bias.data[:] = 1.0  # Keep this line

            # Ensure s is not zero or too small
            s = max(s, eps)  # Prevent division by zero
            log_value = math.log(5 / (self.nc * (640 / s) ** 2 + eps))  # Add eps to avoid log(0)

            c_list[-1].bias.data[:self.nc] = log_value  # Assign safely
